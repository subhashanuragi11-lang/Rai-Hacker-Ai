...
# ─── CONFIG ───────────────────────────────────────────────────────────────
BOT_TOKEN = "8361111114:AAF4bbPSk9A_pTTof1c_Ulc_V4HuTCT20DI"
ADMIN_ID = 6406769029
...
async def query_huggingface(prompt: str) -> str:
    ...
    try:
        async with aiohttp.ClientSession() as sess:
            async with sess.post(HF_API_URL, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                if resp.status == 200:
...
async def handle_text(message: Message):
    ...
    response = await query_huggingface(prompt)
