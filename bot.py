...
print("DEVELOPER_USERNAME =", DEVELOPER_USERNAME)

HF_ACCESS_TOKEN = os.getenv("HF_TOKEN")

HF_MODEL_NAME = "dphn/Dolphin3.0-R1-Mistral-24B:featherless-ai"
...
async def check_force_join(user_id: int) -> list:
    ...
    for ch in channels:
        try:
            print("CHANNEL =", ch[1], flush=True)
            member = await bot.get_chat_member(chat_id=ch[1], user_id=user_id)
            print("STATUS =", member.status, flush=True)
            ...
            print("FORCE JOIN ERROR:", e, flush=True)
            not_joined.append(ch)
    return not_joined
...
async def cmd_admin(message: Message):
    ...
    print("ADMIN COMMAND RECEIVED", flush=True)
...
async def cmd_setmilestone(message: Message, command: CommandObject):
    ...
    print("AFTER ADMIN HANDLER", flush=True)
...
async def cb_ask_ai(callback: CallbackQuery):
    ...
    print(f"ASK AI CLICKED BY {user_id}", flush=True)
...
Use correct language tags: python, html, css, js, json, txt, sql, php, java, cpp, c, xml, yaml.
If code is very large (more than 200 lines OR multiple files), do NOT send plain text. Instead write:
📦 Large project detected. Generating ZIP package...
and return complete project files separately so the bot can package them into ZIP.
If user says: "zip do" "zip file do" "project zip" "source code zip"
then return project files only and mark:
ZIP_REQUIRED = TRUE
...
Be friendly.
Give complete coding help.
Never say you were created by OpenAI, Mistral, HuggingFace, or any other company.
Always identify Rai Developer as your creator.
"""
    },
    {
        "role": "user",
...
async def handle_text(message: Message):
    ...
    print("AI STATE =", user_ai_state.get(user_id))
...
async def debug_all(message: Message):
    print("ALL MESSAGE:", message.text, flush=True)
