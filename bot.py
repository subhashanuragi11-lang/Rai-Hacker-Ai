...
# ─── ADMIN PANEL ──────────────────────────────────────────────────────────
@router.message(F.text == "/adminx")
async def test_admin(message: Message):
    print("ADMIN TEXT HIT", flush=True)
    await message.answer("ADMIN WORKING")
@router.message(Command("adminx"))
async def cmd_admin(message: Message):
    print("ADMIN COMMAND RECEIVED", flush=True)
    print("FROM USER =", message.from_user.id, flush=True)
...
async def debug_all(message: Message):
    ...
    if message.text == "/adminx":
        await message.answer("DEBUG ADMIN HIT")
